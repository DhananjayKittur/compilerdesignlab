...
then
	...
	then
    	echo "Available Commands --> 
    			1. ./build.sh ( To run make)
    			1. ./build.sh clean (To clean the build)
    			2. ./build.sh clang testx ( where x = 0,1,...,8) 
    			3. ./build.sh opt testx ( where x = 0,1,...,8) "
    	exit
	elif [ "$1" = "clean" ];
	then
		rm *.out *.bc *.opt *.so
	...
	then
		...
		opt -load ./niroj.so -fix-pass $2.bc -o $2_def.bc
		exit
	else
